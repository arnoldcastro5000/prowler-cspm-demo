...
| LLM09 | Misinformation | 🟢 Mitigated | 13 CI checks; Prowler before/after as IaC integration test; OWASP ZAP; single-dev review; Zod runtime validation |
...
- 13 CI checks run on every push and PR, including type checking, linting, security scanning (Bandit, Semgrep, Trivy, Gitleaks), and build verification.
...
Partially addresses **LLM-R02** (Supply Chain Vulnerabilities). Mirrors `docs/owasp-genai.md` → Remediation log RL-03.

**Implemented (2026-05-30):** Socket.dev installed as a GitHub App. Scans npm package manifests (package.json, package-lock.json) for malware, typosquatting, obfuscated code, and other supply-chain compromise indicators on every PR — adding behavioural signal detection that complements Dependency Review's CVE-based checks.

**LLM-R02 status:** install-script execution path closed (RL-02); Semgrep gates first-party JS/TS (RL-01); Socket.dev adds behavioural manifest scanning (RL-03). The novel-typosquat-in-initial-selection core remains an **accepted residual** — no detection mechanism exists for an unreported typosquat already in `package-lock.json`.

### RL-04 — lockfile-lint added to Frontend CI (lockfile integrity validation)

Partially addresses **LLM-R02** (Supply Chain Vulnerabilities). Mirrors `docs/owasp-genai.md` → Remediation log RL-04.
...
**LLM-R02 status:** install-script execution path closed (RL-02); Semgrep gates first-party JS/TS (RL-01); Socket.dev adds behavioural manifest scanning (RL-03); lockfile-lint validates registry source and integrity hashes (RL-04). The novel-typosquat-in-initial-selection core remains an **accepted residual** — lockfile-lint verifies where packages resolve from but cannot detect a legitimately sourced package that is itself malicious.
