...
| **2. Build & supply chain** | Compromised dependency, malicious commit, CI runner takeover | 11 CI gates, SHA-pinned actions, `persist-credentials: false`, Dependabot, Zizmor | Zero-day in a pinned dependency before the next Dependabot PR |
...
| Malicious commit is pushed to the repository | Bad code reaches the deployed dashboard | 11 automated CI checks run on every push and PR; all GitHub Actions are pinned to commit SHAs (not mutable tags); deployment is a manual step from the developer workstation, never auto-deployed from CI |
