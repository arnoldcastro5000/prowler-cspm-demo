...
setup:
	...
	terraform -chdir=$(ENV_DIR) apply -target=module.aws -var-file=after.tfvars $(TF_VARS) -var="azure_subscription_id=$$AZURE_SUB_ID" -auto-approve && \
	echo "=== [2/3] Creating GCP resources ===" && \
	terraform -chdir=$(ENV_DIR) apply -target=module.gcp -var-file=after.tfvars $(TF_VARS) -var="azure_subscription_id=$$AZURE_SUB_ID" -auto-approve && \
	echo "=== [3/3] Creating Azure resources ===" && \
	terraform -chdir=$(ENV_DIR) apply -target=module.azure -var-file=after.tfvars $(TF_VARS) -var="azure_subscription_id=$$AZURE_SUB_ID" -auto-approve && \
...
before:
	...
	terraform -chdir=$(ENV_DIR) apply -target=module.aws -var-file=before.tfvars $(TF_VARS) -var="azure_subscription_id=$$AZURE_SUB_ID" -auto-approve && \
	echo "=== [2/3] Misconfiguring GCP resources ===" && \
	terraform -chdir=$(ENV_DIR) apply -target=module.gcp -var-file=before.tfvars $(TF_VARS) -var="azure_subscription_id=$$AZURE_SUB_ID" -auto-approve && \
	echo "=== [3/3] Misconfiguring Azure resources ===" && \
	terraform -chdir=$(ENV_DIR) apply -target=module.azure -var-file=before.tfvars $(TF_VARS) -var="azure_subscription_id=$$AZURE_SUB_ID" -auto-approve
