...
.PHONY: before scan after rescan deploy _prowl

# ─── before: apply misconfigured state, start EC2 ────────────────────────────
before:
	@echo "=== Applying before (misconfigured) state ==="
	terraform -chdir=$(ENV_DIR) apply -var-file=before.tfvars -auto-approve

# ─── after: apply hardened state, stop EC2 ───────────────────────────────────
after:
	@echo "=== Applying after (hardened) state ==="
	terraform -chdir=$(ENV_DIR) apply -var-file=after.tfvars -auto-approve
